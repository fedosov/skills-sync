...
type DotagentsWorkspaceProps = {
  onReady?: () => void;
};

export function DotagentsWorkspace({ onReady }: DotagentsWorkspaceProps = {}) {
  ...
  async function handleSync() {
    await runBusyAction("sync", async () => {
      ...
      if (!result.success) {
        setError(commandFailureMessage(result, "dotagents command failed"));
        return;
      }
      await refreshApp();
    });
    ...
    return (
      <div className="space-y-8">
        ...
        <OutputPanel
          ...
          subtitle="Latest vendor command transcript"
          emptyMessage="Run sync or remove to capture a transcript here."
        />
      </div>
    );
  }
